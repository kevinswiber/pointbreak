//! One-shot migration driver: converge a reshaped store onto the final
//! `object`-shaped wire in a single pass. Owner-run; NOT part of the `shore`
...
use shoreline::session::{MigrateOptions, migrate_object_rename};
...
fn main() -> ExitCode {
    ...
    match migrate_object_rename(options) {
        Ok(summary) => {
            println!(
                "migrated {source} -> {target}: events_migrated={} events_passed_through={} \
                 content_ids_rederived={} artifacts_rehashed={} inline_signatures_resigned={} \
                 cosignatures_reattested={} cosignatures_dropped={} self_check_passed={}",
