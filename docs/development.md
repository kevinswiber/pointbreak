...
| Developer-only LMDB proof closure | Focused closure tests plus `cargo bench --locked --features bench,lmdb-proof --bench store_foundation -- --lmdb-proof-open-close` | Compile every `.github/binary-targets.json` target; run plain open/close and native dependency inspection on representative macOS, Linux glibc, Linux musl, and Windows hosts | Exact reviewed sources compile and link statically; no encryption, adapter semantics, timing, or default-package/release inclusion |
